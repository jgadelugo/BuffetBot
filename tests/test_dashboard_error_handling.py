...
def simulate_dashboard_with_missing_data():
    ...
    missing_data_cases = [
        ...
        {"price_data": None, "fundamentals": None, "metrics": None},
    ...
    for i, data in enumerate(missing_data_cases, 1):
        ...
        # Price Change
        price_change = safe_format_percentage(
            safe_get_nested_value(data, "metrics", "price_change")
        )
        print(f"  Price Change: {price_change}")

        # Market Cap
        market_cap = safe_format_currency(
            safe_get_nested_value(data, "fundamentals", "market_cap")
        )
        print(f"  Market Cap: {market_cap}")

        # P/E Ratio
        pe_ratio = safe_format_number(
            safe_get_nested_value(data, "fundamentals", "pe_ratio")
        )
        print(f"  P/E Ratio: {pe_ratio}")

        # Volatility
        volatility = safe_format_percentage(
...
def simulate_risk_metrics_with_none():
    ...
    risk_results = [
        ...
        {},  # Empty result
        {"overall_risk": None},
        {"overall_risk": {"score": None, "level": "Unknown"}},
        {"overall_risk": {"score": 65.5, "level": "Moderate"}},
    ]

...
def _calculate_score(
    ...
    health_metrics: Dict[str, Union[int, float, str, List[str]]],
    growth_metrics: Dict[str, Union[float, Dict[str, float]]]
) -> Dict[str, Any]:
    """
    ...
    try:
        score = 0
        score_breakdown = {category: 0 for category in ['value', 'health', 'growth', 'profitability']}
        reasoning = []
        
        # Value metrics scoring
        for metric, rule in {k: v for k, v in SCORING_RULES.items() if v['category'] == 'value'}.items():
        ...
        for metric, rule in {k: v for k, v in SCORING_RULES.items() if v['category'] == 'health'}.items():
        ...
        for metric, rule in {k: v for k, v in SCORING_RULES.items() if v['category'] == 'growth'}.items():
        ...
        for metric, rule in {k: v for k, v in SCORING_RULES.items() if v['category'] == 'profitability'}.items():
            if metric in value_metrics:  # Profitability metrics are stored in value_metrics
                value = value_metrics[metric]
                if value >= rule['threshold']:
                    score += rule['points']
...
def generate_recommendation(
    ...
    try:
        ...
        # Extract key metrics
        metrics_used = _extract_key_metrics(value_metrics, health_metrics, growth_metrics)
        
        # Log recommendation
        logger.info(f"Generated recommendation: {recommendation} (Score: {score_results['total_score']})")
    ...
    try:
        ...
        detailed_analysis = {
            'value': {
                ...
                'max_score': sum(rule['points'] for rule in SCORING_RULES.values() if rule['category'] == 'value'),
                'metrics': {
            ...
            'health': {
                ...
                'max_score': sum(rule['points'] for rule in SCORING_RULES.values() if rule['category'] == 'health'),
                'metrics': {
            ...
            'growth': {
                ...
                'max_score': sum(rule['points'] for rule in SCORING_RULES.values() if rule['category'] == 'growth'),
                'metrics': {
            ...
            'profitability': {
                ...
                'max_score': sum(rule['points'] for rule in SCORING_RULES.values() if rule['category'] == 'profitability'),
                'metrics': {

    ...
    try:
        ...
        with col2:
            if 'altman_z_score' in health_metrics_result:
                ...
                status = 'good' if score > 2.99 else 'warning' if score > 1.81 else 'bad'
                display_metric_with_status(
    ...
    try:
        ...
        def create_ratio_table(metrics_dict: Dict) -> None:
            ...
            for name, info in metrics_dict.items():
                ...
                if value is None or pd.isna(value) or (isinstance(value, (int, float)) and value == 0 and 'calculation_failed' in info):
                    formatted_value = "N/A"
                    status_emoji = "❓"
                else:
        ...
        with tab1:
            ...
            st.markdown("Measures the company's ability to meet its short-term obligations")
            ...
            liquidity_metrics = {
                'Current Ratio': {
                    ...
                    'type': 'ratio',
                    'status': 'good' if ratios.get('current_ratio', 0) > 1 else 'warning',
                    'help_text': 'Measures ability to pay short-term obligations',
                    'calculation_failed': not calculation_status.get('current_ratio', True)
                },
                'Quick Ratio': {
                    'value': ratios.get('quick_ratio', 0),
                    'metric_key': 'quick_ratio',
                    'type': 'ratio',
                    'status': 'good' if ratios.get('quick_ratio', 0) > 0.8 else 'warning',
                    'help_text': 'Measures ability to meet short-term obligations with most liquid assets',
                    'calculation_failed': not calculation_status.get('quick_ratio', True)
                },
                'Cash Ratio': {
                    'value': ratios.get('cash_ratio', 0),
                    'metric_key': 'cash_ratio',
                    'type': 'ratio',
                    'status': 'good' if ratios.get('cash_ratio', 0) > 0.5 else 'warning',
                    'help_text': 'Measures ability to pay short-term obligations with cash and cash equivalents',
                    'calculation_failed': not calculation_status.get('cash_ratio', True)
                }
            }
        ...
        with tab2:
            ...
            leverage_metrics = {
                'Debt to Equity': {
                    ...
                    'type': 'ratio',
                    'status': 'good' if ratios.get('debt_to_equity', 0) < 2 else 'warning',
                    'help_text': 'Measures financial leverage and risk',
                    'calculation_failed': not calculation_status.get('debt_to_equity', True)
                },
                'Debt to Assets': {
                    'value': ratios.get('debt_to_assets', 0),
                    'metric_key': 'debt_to_assets',
                    'type': 'ratio',
                    'status': 'good' if ratios.get('debt_to_assets', 0) < 0.5 else 'warning',
                    'help_text': 'Measures percentage of assets financed by debt',
                    'calculation_failed': not calculation_status.get('debt_to_assets', True)
                },
                'Interest Coverage': {
                    'value': ratios.get('interest_coverage', 0),
                    'metric_key': 'interest_coverage',
                    'type': 'ratio',
                    'status': 'good' if ratios.get('interest_coverage', 0) > 3 else 'warning',
                    'help_text': 'Measures ability to pay interest on debt',
                    'calculation_failed': not calculation_status.get('interest_coverage', True)
                },
                'Long-term Debt to Equity': {
                    'value': ratios.get('long_term_debt_to_equity', 0),
                    'metric_key': 'long_term_debt_to_equity',
                    'type': 'ratio',
                    'status': 'good' if ratios.get('long_term_debt_to_equity', 0) < 1 else 'warning',
                    'help_text': 'Measures long-term financial leverage',
                    'calculation_failed': not calculation_status.get('long_term_debt_to_equity', True)
                }
            }
        ...
        with tab3:
            ...
            profitability_metrics = {
                'Return on Equity': {
                    ...
                    'type': 'percentage',
                    'status': 'good' if ratios.get('return_on_equity', 0) > 0.15 else 'warning',
                    'help_text': 'Measures profitability relative to shareholder equity',
                    'calculation_failed': not calculation_status.get('return_on_equity', True)
                },
                'Return on Assets': {
                    'value': ratios.get('return_on_assets', 0),
                    'metric_key': 'return_on_assets',
                    'type': 'percentage',
                    'status': 'good' if ratios.get('return_on_assets', 0) > 0.05 else 'warning',
                    'help_text': 'Measures profitability relative to total assets',
                    'calculation_failed': not calculation_status.get('return_on_assets', True)
                },
                'Gross Margin': {
                    'value': ratios.get('gross_margin', 0),
                    'metric_key': 'gross_margin',
                    'type': 'percentage',
                    'status': 'good' if ratios.get('gross_margin', 0) > 0.3 else 'warning',
                    'help_text': 'Measures profitability after cost of goods sold',
                    'calculation_failed': not calculation_status.get('gross_margin', True)
                },
                'Operating Margin': {
                    'value': ratios.get('operating_margin', 0),
                    'metric_key': 'operating_margin',
                    'type': 'percentage',
                    'status': 'good' if ratios.get('operating_margin', 0) > 0.15 else 'warning',
                    'help_text': 'Measures profitability from operations',
                    'calculation_failed': not calculation_status.get('operating_margin', True)
                },
                'Net Margin': {
                    'value': ratios.get('net_margin', 0),
                    'metric_key': 'net_margin',
                    'type': 'percentage',
                    'status': 'good' if ratios.get('net_margin', 0) > 0.1 else 'warning',
                    'help_text': 'Measures overall profitability',
                    'calculation_failed': not calculation_status.get('net_margin', True)
                }
            }
...
def render_health_indicators(health_metrics_result: Dict) -> None:
    ...
    try:
        ...
        if health_flags:
            ...
            critical_flags = [flag for flag in health_flags if 'critical' in flag.lower()]
            warning_flags = [flag for flag in health_flags if 'warning' in flag.lower()]
            info_flags = [flag for flag in health_flags if flag not in critical_flags + warning_flags]

...
def render_valuation_overview(
    ...
    try:
        ...
        logger.info(f"Input data - current_price: {current_price}, intrinsic_value: {intrinsic_value}")
        logger.info(f"Intrinsic value result keys: {list(intrinsic_value_result.keys())}")
        ...
        try:
            ...
            assumptions_metrics = {
                'Growth Rate': {
                    ...
                    'status': 'good' if assumptions.get('growth_rate', 0) > 0.05 else 'warning',
        ...
        try:
            ...
            metrics_normalized = {
                ...
                'Growth Score': min(data['fundamentals'].get('revenue_growth', 0) * 500, 100) if data['fundamentals'].get('revenue_growth', 0) > 0 else 50,
                'Quality Score': min(data['fundamentals'].get('roe', 0) * 300, 100) if data['fundamentals'].get('roe', 0) > 0 else 50,
                'Financial Health': 80 if data['fundamentals'].get('debt_to_equity', 1) < 0.5 else 50,
                'Momentum': min(data['metrics'].get('momentum', 0) * 200 + 50, 100) if data['metrics'].get('momentum', 0) > 0 else 50
            }
            logger.info(f"Normalized metrics: {metrics_normalized}")
            
            radar_chart = create_valuation_metrics_chart(metrics_normalized)
            st.plotly_chart(radar_chart, use_container_width=True, key="valuation_metrics_radar")
            logger.info("Valuation metrics radar chart created successfully")
        except Exception as e:
            logger.error(f"Error creating valuation metrics chart: {str(e)}", exc_info=True)
...
def render_technical_analysis(price_data: pd.DataFrame) -> None:
    ...
    with col2:
        ...
        chart_type = st.radio(
            "Chart Type",
            ['candlestick', 'line'],
            index=0 if all(col in price_data.columns for col in ['Open', 'High', 'Low', 'Close']) else 1
        )
    ...
    with col1:
        ...
        st.plotly_chart(tech_chart, use_container_width=True, key="technical_analysis_chart")
    ...
    try:
        ...
        price_metrics = {
            ...
            'RSI': {
                ...
                'status': 'warning' if data['metrics']['rsi'] > 70 or data['metrics']['rsi'] < 30 else 'good',
                'metric_key': 'rsi',
                'type': 'score'
            },
            'Momentum': {
        ...
        if 'valuation_breakdown' in intrinsic_value_result:
            ...
            breakdown_df['Value'] = breakdown_df['Value'].apply(lambda x: f"${x:.2f}" if x else "N/A")
...
def render_investment_summary(
    ...
    with col1:
        display_metric_with_status(
            ...
            status='good' if upside_potential > 0.15 else 'warning' if upside_potential > 0 else 'bad',
    ...
    with col2:
        display_metric_with_status(
            ...
            status='good' if margin_of_safety > 0.15 else 'warning' if margin_of_safety > 0 else 'bad',
    ...
    with col3:
        display_metric_with_status(
            ...
            status='good' if data['metrics']['volatility'] < 0.2 else 'warning' if data['metrics']['volatility'] < 0.4 else 'bad',
...
def render_basic_price_metrics(data: Dict) -> None:
    ...
    with col2:
        st.metric(
            "52-Week High",
            f"${data['price_data']['Close'].tail(252).max():.2f}"
        )
    
    with col3:
        st.metric(
            "52-Week Low",
            f"${data['price_data']['Close'].tail(252).min():.2f}"
        )
...
def calculate_technical_signals(price_data: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        if len(price_data) >= 20:
            recent_trend = (price_data['Close'].iloc[-1] - price_data['Close'].iloc[-20]) / price_data['Close'].iloc[-20]
...
def generate_investment_thesis(
    ...
    if margin_of_safety >= 0.15:
        thesis.append(f"Stock appears undervalued with {margin_of_safety:.1%} margin of safety")
    elif margin_of_safety >= 0:
        thesis.append(f"Stock is fairly valued with limited {margin_of_safety:.1%} margin of safety")
    else:
        thesis.append(f"Stock appears overvalued by {abs(margin_of_safety):.1%}")
    
    # Growth thesis
    if fundamentals.get('revenue_growth', 0) > 0.15:
        thesis.append(f"Strong revenue growth of {fundamentals['revenue_growth']:.1%} indicates business expansion")
    elif fundamentals.get('revenue_growth', 0) > 0.05:
        thesis.append(f"Moderate revenue growth of {fundamentals['revenue_growth']:.1%} shows stable business")
    
    # Quality thesis
    if fundamentals.get('roe', 0) > 0.15:
        thesis.append(f"High ROE of {fundamentals['roe']:.1%} demonstrates efficient capital utilization")
    
    # Financial health thesis
    if fundamentals.get('debt_to_equity', 1) < 0.5:
        thesis.append("Strong balance sheet with low debt levels provides financial flexibility")
    ...
    if metrics.get('momentum', 0) > 0.1:
        thesis.append(f"Positive price momentum of {metrics['momentum']:.1%} suggests market confidence")
    elif metrics.get('momentum', 0) < -0.1:
        thesis.append(f"Negative momentum of {metrics['momentum']:.1%} may indicate market concerns")
    
    # Risk thesis
    if metrics.get('volatility', 0) > 0.3:
        thesis.append(f"High volatility of {metrics['volatility']:.1%} suggests increased investment risk")

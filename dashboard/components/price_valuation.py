...
class PriceValuationCard:
    ...
    def _calculate_margin_of_safety(self) -> Optional[float]:
    ...
    def _determine_valuation_status(self) -> Dict[str, any]:
        ...
        price_diff_pct = ((self.current_price - self.intrinsic_value) / self.intrinsic_value) * 100
    ...
    def render(self):
        ...
        with col3:
            margin_text = f"{self.margin_of_safety:.1%}" if self.margin_of_safety is not None else "N/A"
            # Color the delta based on margin of safety
            delta_color = "normal" if self.margin_of_safety and self.margin_of_safety > 0 else "inverse"
            st.metric(
...
def create_valuation_summary(
    ...
    try:
        ...
        margin_of_safety = (intrinsic_value - current_price) / intrinsic_value if intrinsic_value > 0 else None
        ...
        overall_score = sum(score_components) / len(score_components) if score_components else 50

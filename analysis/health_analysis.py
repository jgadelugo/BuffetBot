...
def _validate_financial_data(financials: pd.DataFrame, required_columns: List[str]) -> bool:
    """
...
def _calculate_financial_ratio(
    ...
    try:
        if numerator not in financials.columns or denominator not in financials.columns:
            return default, f"Missing columns for ratio calculation: {numerator}/{denominator}"
...
def calculate_piotroski_score(income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame, cash_flow: pd.DataFrame) -> int:
    """
    ...
    try:
        ...
        if income_stmt is not None and not income_stmt.empty:
            # Net Income > 0
            if 'Net Income' in income_stmt.columns and income_stmt['Net Income'].iloc[0] > 0:
                score += 1
            
            # Operating Cash Flow > 0
            if cash_flow is not None and 'Operating Cash Flow' in cash_flow.columns and cash_flow['Operating Cash Flow'].iloc[0] > 0:
                score += 1
        ...
        if balance_sheet is not None and not balance_sheet.empty:
            ...
            if 'Long Term Debt' in balance_sheet.columns and 'Total Assets' in balance_sheet.columns:
                total_assets = balance_sheet['Total Assets'].iloc[0]
                if total_assets != 0:  # Check for division by zero
                    ...
                    logger.warning("Total Assets is zero, skipping debt ratio calculation")
            ...
            if 'Total Current Assets' in balance_sheet.columns and 'Total Current Liabilities' in balance_sheet.columns:
                ...
                if current_liabilities != 0:  # Check for division by zero
                    current_ratio = balance_sheet['Total Current Assets'].iloc[0] / current_liabilities
                    if current_ratio > 1:
                        score += 1
                else:
                    logger.warning("Current Liabilities is zero, skipping current ratio calculation")
        ...
        if income_stmt is not None and not income_stmt.empty:
            ...
            if 'Gross Profit' in income_stmt.columns and 'Total Revenue' in income_stmt.columns:
                ...
                if revenue != 0:  # Check for division by zero
                    ...
                    logger.warning("Total Revenue is zero, skipping gross margin calculation")
            ...
            if 'Total Revenue' in income_stmt.columns and 'Total Assets' in balance_sheet.columns:
                ...
                if total_assets != 0:  # Check for division by zero
                    ...
                    logger.warning("Total Assets is zero, skipping asset turnover calculation")
...
def calculate_altman_z_score(balance_sheet: pd.DataFrame, income_stmt: pd.DataFrame) -> float:
    """
    ...
    try:
        ...
        # Required columns
        required_cols = {
            'balance_sheet': ['Total Assets', 'Total Current Assets', 'Total Current Liabilities', 
        ...
        for statement, cols in required_cols.items():
            ...
            if df is None or df.empty:
                ...
                if statement == 'income_stmt' and 'EBIT' not in df.columns and 'Operating Income' in df.columns:
                    # Use Operating Income as EBIT
                    income_stmt = income_stmt.copy()
                    income_stmt['EBIT'] = income_stmt['Operating Income']
                    logger.info("Using Operating Income as EBIT for Altman Z-Score calculation")
                else:
                    missing_cols.extend([col for col in cols if col not in df.columns])
        
        if missing_cols:
            logger.warning(f"Missing required columns for Altman Z-Score: {', '.join(missing_cols)}")
            return 0.0
        ...
        try:
            X1 = (balance_sheet['Total Current Assets'].iloc[0] - balance_sheet['Total Current Liabilities'].iloc[0]) / total_assets
...
def calculate_financial_ratios(income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame, cash_flow: pd.DataFrame) -> Dict[str, float]:
    """
    ...
    try:
        ...
        if balance_sheet is not None and not balance_sheet.empty:
            if 'Total Current Assets' in balance_sheet.columns and 'Total Current Liabilities' in balance_sheet.columns:
                ...
                if current_liabilities != 0:
                    ratios['current_ratio'] = balance_sheet['Total Current Assets'].iloc[0] / current_liabilities
                else:
                    logger.warning("Current Liabilities is zero, skipping current ratio calculation")
        ...
        if balance_sheet is not None and not balance_sheet.empty:
            if 'Total Liabilities' in balance_sheet.columns and 'Total Stockholder Equity' in balance_sheet.columns:
                ...
                if total_equity != 0:
                    ratios['debt_to_equity'] = balance_sheet['Total Liabilities'].iloc[0] / total_equity
                else:
                    logger.warning("Total Stockholder Equity is zero, skipping debt to equity calculation")
        
        # Debt to assets
        if balance_sheet is not None and not balance_sheet.empty:
            if 'Total Liabilities' in balance_sheet.columns and 'Total Assets' in balance_sheet.columns:
                total_assets = balance_sheet['Total Assets'].iloc[0]
                if total_assets != 0:
                    ratios['debt_to_assets'] = balance_sheet['Total Liabilities'].iloc[0] / total_assets
                else:
                    logger.warning("Total Assets is zero, skipping debt to assets calculation")
        ...
        if income_stmt is not None and not income_stmt.empty:
            if 'Operating Income' in income_stmt.columns and 'Interest Expense' in income_stmt.columns:
                ...
                if pd.isna(interest_expense) and len(income_stmt) > 1:
                    for i in range(1, min(len(income_stmt), 3)):  # Check up to 2 previous years
                        ...
                        if not pd.isna(alt_interest) and alt_interest != 0:
                            ...
                            logger.info(f"Using interest expense from year {i} back: {interest_expense}")
                            break
                ...
                if not pd.isna(interest_expense) and interest_expense != 0:
                    ...
                    logger.warning("Interest Expense is zero, skipping interest coverage calculation")
                else:
                    logger.warning("Interest Expense is NaN, skipping interest coverage calculation")
        ...
        if income_stmt is not None and not income_stmt.empty and balance_sheet is not None and not balance_sheet.empty:
            if 'Net Income' in income_stmt.columns and 'Total Stockholder Equity' in balance_sheet.columns:
                total_equity = balance_sheet['Total Stockholder Equity'].iloc[0]
                if total_equity != 0:
                    ratios['return_on_equity'] = income_stmt['Net Income'].iloc[0] / total_equity
                else:
                    logger.warning("Total Stockholder Equity is zero, skipping return on equity calculation")
        ...
        if income_stmt is not None and not income_stmt.empty and balance_sheet is not None and not balance_sheet.empty:
            if 'Net Income' in income_stmt.columns and 'Total Assets' in balance_sheet.columns:
                total_assets = balance_sheet['Total Assets'].iloc[0]
                if total_assets != 0:
                    ratios['return_on_assets'] = income_stmt['Net Income'].iloc[0] / total_assets
                else:
                    logger.warning("Total Assets is zero, skipping return on assets calculation")
        ...
        if income_stmt is not None and not income_stmt.empty:
            if 'Gross Profit' in income_stmt.columns and 'Total Revenue' in income_stmt.columns:
                revenue = income_stmt['Total Revenue'].iloc[0]
                if revenue != 0:
                    ratios['gross_margin'] = income_stmt['Gross Profit'].iloc[0] / revenue
                else:
                    logger.warning("Total Revenue is zero, skipping gross margin calculation")
        ...
        if income_stmt is not None and not income_stmt.empty:
            if 'Operating Income' in income_stmt.columns and 'Total Revenue' in income_stmt.columns:
                revenue = income_stmt['Total Revenue'].iloc[0]
                if revenue != 0:
                    ratios['operating_margin'] = income_stmt['Operating Income'].iloc[0] / revenue
                else:
                    logger.warning("Total Revenue is zero, skipping operating margin calculation")
        ...
        if income_stmt is not None and not income_stmt.empty:
            if 'Net Income' in income_stmt.columns and 'Total Revenue' in income_stmt.columns:
                ...
                if revenue != 0:
                    ...
                    logger.warning("Total Revenue is zero, skipping net margin calculation")
...
def analyze_financial_health(data: Dict) -> Dict:
    ...
    try:
        ...
        try:
            result['piotroski_score'] = calculate_piotroski_score(income_stmt, balance_sheet, cash_flow)
        except Exception as e:
            logger.error(f"Error calculating Piotroski score: {str(e)}", exc_info=True)
        
        # Calculate Altman Z-Score
        try:
            result['altman_z_score'] = calculate_altman_z_score(balance_sheet, income_stmt)
        except Exception as e:
            logger.error(f"Error calculating Altman Z-score: {str(e)}", exc_info=True)
        
        # Calculate financial ratios
        try:
            result['financial_ratios'] = calculate_financial_ratios(income_stmt, balance_sheet, cash_flow)
        except Exception as e:
            logger.error(f"Error calculating financial ratios: {str(e)}", exc_info=True)

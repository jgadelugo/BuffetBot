    ...
    try:
        ...
        try:
            ...
            if beta is not None:
                ...
                if beta > 1.5:
                    result['overall_risk']['factors'].append("High market sensitivity (beta > 1.5)")
                elif beta < 0.5:
                    result['overall_risk']['factors'].append("Low market sensitivity (beta < 0.5)")
            else:
                result['overall_risk']['warnings'].append("Beta not available in fundamentals data")
            ...
            if 'price_data' in data and data['price_data'] is not None:
                try:
                    ...
                    if len(returns) < 2:
                        result['overall_risk']['warnings'].append("Insufficient price data for volatility calculation")
                    else:
                        volatility = returns.std() * np.sqrt(252)  # Annualized volatility
                        market_risk['volatility'] = volatility
                        if volatility > 0.4:
                            result['overall_risk']['factors'].append("High price volatility (> 40%)")
                        elif volatility < 0.1:
                            result['overall_risk']['factors'].append("Low price volatility (< 10%)")
                except Exception as e:
                    error_msg = f"Error calculating volatility: {str(e)}"
                    logger.error(error_msg)
                    result['overall_risk']['errors'].append(error_msg)
            else:
                result['overall_risk']['warnings'].append("Price data not available for volatility calculation")
        ...
        try:
            ...
            if balance_sheet is None or balance_sheet.empty:
                result['overall_risk']['warnings'].append("Balance sheet data not available")
            else:
                # Debt to equity
                if 'Total Liabilities' in balance_sheet.columns and 'Total Stockholder Equity' in balance_sheet.columns:
                    ...
                    if total_equity != 0:
                        ...
                        if debt_to_equity > 1.0:
                            result['overall_risk']['factors'].append("High leverage (D/E > 1.0)")
                    else:
                        result['overall_risk']['warnings'].append("Total Stockholder Equity is zero")
                else:
                    result['overall_risk']['warnings'].append("Missing required columns for debt-to-equity calculation")
                
                # Interest coverage
                if income_stmt is not None and not income_stmt.empty:
                    if 'Operating Income' in income_stmt.columns and 'Interest Expense' in income_stmt.columns:
                        ...
                        if interest_expense != 0:
                            ...
                            if interest_coverage < 1.0:
                                result['overall_risk']['factors'].append("Low interest coverage (< 1.0)")
                        else:
                            result['overall_risk']['warnings'].append("Interest Expense is zero")
                    else:
                        result['overall_risk']['warnings'].append("Missing required columns for interest coverage calculation")
                else:
                    result['overall_risk']['warnings'].append("Income statement data not available")
                ...
                if 'Operating Income' in income_stmt.columns and 'Total Revenue' in income_stmt.columns:
                    ...
                    if revenue != 0:
                        ...
                        if operating_margin < 0.1:
                            result['overall_risk']['factors'].append("Low operating margin (< 10%)")
                    else:
                        result['overall_risk']['warnings'].append("Total Revenue is zero")
                else:
                    result['overall_risk']['warnings'].append("Missing required columns for operating margin calculation")
                ...
                if 'Total Revenue' in income_stmt.columns:
                    ...
                    if revenue > 0:
                        ...
                        if revenue < 1e9:  # Less than $1B
                            result['overall_risk']['factors'].append("Small revenue base (< $1B)")
                    else:
                        result['overall_risk']['warnings'].append("Total Revenue is zero or negative")
                else:
                    result['overall_risk']['warnings'].append("Total Revenue column not found")
        ...
        try:
            ...
            if result['market_risk']:
                ...
                market_score = (min(abs(beta - 1.0) * 2, 2.0) / 2.0 + min(volatility * 2, 1.0)) / 2
                
                # Add detailed explanations for market risk
                if beta > 1.5:
                    result['overall_risk']['factors'].append(f"Beta of {beta:.2f} indicates high volatility relative to market")
                elif beta < 0.5:
                    result['overall_risk']['factors'].append(f"Beta of {beta:.2f} indicates low correlation with market")
                
                if volatility > 0.4:
                    result['overall_risk']['factors'].append(f"Annual volatility of {volatility:.1%} is very high")
                elif volatility > 0.3:
                    result['overall_risk']['factors'].append(f"Annual volatility of {volatility:.1%} is above average")
            else:
                result['overall_risk']['warnings'].append("Market risk metrics not available for score calculation")
            
            financial_score = 0.0
            if result['financial_risk']:
                debt_to_equity = result['financial_risk'].get('debt_to_equity', 0.5)
                interest_coverage = result['financial_risk'].get('interest_coverage', 3.0)
                # Higher debt and lower coverage = higher risk
                financial_score = (min(debt_to_equity, 2.0) / 2.0 + max(0, 1 - interest_coverage / 5.0)) / 2
                
                # Add detailed explanations for financial risk
                if debt_to_equity > 2.0:
                    result['overall_risk']['factors'].append(f"Debt-to-equity ratio of {debt_to_equity:.2f} indicates very high leverage")
                elif debt_to_equity > 1.0:
                    result['overall_risk']['factors'].append(f"Debt-to-equity ratio of {debt_to_equity:.2f} indicates significant leverage")
                
                if interest_coverage < 1.5:
                    result['overall_risk']['factors'].append(f"Interest coverage of {interest_coverage:.2f}x is dangerously low")
                elif interest_coverage < 3.0:
                    result['overall_risk']['factors'].append(f"Interest coverage of {interest_coverage:.2f}x may be insufficient in downturns")
            else:
                result['overall_risk']['warnings'].append("Financial risk metrics not available for score calculation")
            ...
            if result['business_risk']:
                ...
                business_score = (max(0, 1 - operating_margin / 0.3) + max(0, 1 - revenue / 1e10)) / 2
                
                # Add detailed explanations for business risk
                if operating_margin < 0.05:
                    result['overall_risk']['factors'].append(f"Operating margin of {operating_margin:.1%} is very low")
                elif operating_margin < 0.10:
                    result['overall_risk']['factors'].append(f"Operating margin of {operating_margin:.1%} is below healthy levels")
                
                if revenue < 1e8:  # Less than $100M
                    result['overall_risk']['factors'].append(f"Small company with revenue of ${revenue/1e6:.0f}M")
                elif revenue < 1e9:  # Less than $1B
                    result['overall_risk']['factors'].append(f"Mid-size company with revenue of ${revenue/1e6:.0f}M")
            else:
                result['overall_risk']['warnings'].append("Business risk metrics not available for score calculation")
            ...
            if overall_score >= 70:
                risk_level = "High"
                result['overall_risk']['factors'].insert(0, "High risk due to combination of market volatility, financial leverage, and business challenges")
            elif overall_score >= 40:
                risk_level = "Moderate"
                result['overall_risk']['factors'].insert(0, "Moderate risk with some concerns in financial health or market exposure")
            else:
                risk_level = "Low"
                result['overall_risk']['factors'].insert(0, "Low risk with stable financials and manageable market exposure")
...
def _calculate_financial_risk(financials: Dict[str, Union[pd.DataFrame, Dict]]) -> Dict[str, float]:
    """
    Calculate financial risk metrics.
    ...
    try:
        ...
        total_equity = balance_sheet.loc['Total Equity'].iloc[0] if 'Total Equity' in balance_sheet.index else 0
...
def _calculate_business_risk(financials: Dict[str, Union[pd.DataFrame, Dict]]) -> Dict[str, float]:
    """
    Calculate business risk metrics.
...
def _calculate_risk_score(
    ...
    try:
        ...
        if business_risk['operating_leverage'] is not None:
            business_score += min(abs(business_risk['operating_leverage']) * 20, 100) * 0.5

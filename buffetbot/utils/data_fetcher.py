...
from buffetbot.data.fetcher.utils.financial_calculations import calculate_rsi
from buffetbot.utils.logger import setup_logger
from buffetbot.utils.validators import validate_date_range, validate_ticker
...
def fetch_stock_data(ticker: str, years: int = 5) -> dict[str, pd.DataFrame | dict]:
    ...
    try:
        ...
        if price_data.empty:
            ...
            logger.info(
                f"Successfully fetched {len(price_data)} days of data for {ticker}"
            )
        ...
        if not price_data.empty:
            ...
            metrics["volatility"] = price_data["Close"].pct_change().std() * np.sqrt(
                252
            )

...
def calculate_intrinsic_value(
    ...
    try:
        ...
        if not isinstance(financials, dict):
            logger.error(
                "Invalid financials data structure: expected dict, got %s",
                type(financials),
            )
        ...
        logger.info(
            f"income_stmt: {type(income_stmt)}, balance_sheet: {type(balance_sheet)}, cash_flow: {type(cash_flow)}"
        )
        if income_stmt is not None:
            logger.info(f"income_stmt columns: {getattr(income_stmt, 'columns', None)}")
        if balance_sheet is not None:
            logger.info(
                f"balance_sheet columns: {getattr(balance_sheet, 'columns', None)}"
            )
        ...
        if missing:
            raise ValueError(
                f"Missing required financial statements: {', '.join(missing)}"
            )
        ...
        if fcf is None:
            logger.error(
                "Could not calculate free cash flow. See previous logs for missing fields and alternatives checked."
            )
            raise ValueError(
                "Could not calculate free cash flow. Check logs for missing fields and alternatives checked."
            )
        ...
        if (
            ...
            try:
                ...
                logger.info(
                    f"Using fallback current price from price_data: {current_price}"
                )
        ...
        if (
            ...
            margin_of_safety = _calculate_margin_of_safety(
                current_price, intrinsic_value
            )
        else:
            logger.warning(
                f"Cannot calculate margin of safety: intrinsic_value={intrinsic_value}, current_price={current_price}"
            )
...
def _calculate_free_cash_flow(
    ...
    try:
        ...
        logger.info(
            f"Cash Flow Statement columns: {cash_flow.columns.tolist() if cash_flow is not None else 'None'}"
        )
        logger.info(
            f"Income Statement columns: {income_stmt.columns.tolist() if income_stmt is not None else 'None'}"
        )
        logger.info(
            f"Balance Sheet columns: {balance_sheet.columns.tolist() if balance_sheet is not None else 'None'}"
        )
        ...
        try:
            ...
            for col in ocf_columns:
                if col in cash_flow.columns:
                    ...
                    operating_cash_flow = (
                        cash_flow[col].iloc[0] if not cash_flow[col].empty else None
                    )
                    logger.info(
                        f"Found operating cash flow in column: {col} = {operating_cash_flow}"
                    )
                    break
            if operating_cash_flow is None:
                logger.error(
                    f"None of the expected operating cash flow columns found for FCF calculation. Tried: {ocf_columns}. Available columns: {list(cash_flow.columns)}"
                )
            capex = None
            ...
            if capex is None:
                logger.error(
                    f"None of the expected capex columns found for FCF calculation. Tried: {capex_columns}. Available columns: {list(cash_flow.columns)}"
                )
            if operating_cash_flow is not None and capex is not None:
                ...
                logger.warning(
                    f"Missing required data for Method 1: operating_cash_flow={operating_cash_flow}, capex={capex}"
                )
        except Exception as e:
            logger.warning(
                f"Failed to calculate FCF from cash flow statement: {str(e)}"
            )
        ...
        try:
            ...
            for col in ebit_columns:
                if col in income_stmt.columns:
                    ebit = (
                        income_stmt[col].iloc[0] if not income_stmt[col].empty else None
                    )
                    logger.info(f"Found EBIT in column: {col} = {ebit}")
                    break
            if ebit is None:
                logger.error(
                    f"None of the expected EBIT columns found for FCF calculation. Tried: {ebit_columns}. Available columns: {list(income_stmt.columns)}"
                )
            if ebit is not None:
                tax_rate = 0.21
                tax_columns = [
                ...
                for col in tax_columns:
                    if col in income_stmt.columns:
                        tax_expense = (
                            income_stmt[col].iloc[0]
                            if not income_stmt[col].empty
                            else None
                        )
                        if tax_expense and ebit != 0:
                            tax_rate = tax_expense / ebit
                        logger.info(
                            f"Found tax rate from column: {col} = {tax_rate:.2%}"
                        )
                        break
                ...
                if cash_flow is not None:
                    for col in dep_columns:
                        if col in cash_flow.columns:
                            depreciation = (
                                cash_flow[col].iloc[0]
                                if not cash_flow[col].empty
                                else 0
                            )
                            logger.info(
                                f"Found depreciation in cash flow column: {col} = {depreciation}"
                            )
                            break
                ...
                if depreciation == 0:
                    for col in dep_columns:
                        if col in income_stmt.columns:
                            ...
                            logger.info(
                                f"Found depreciation in income statement column: {col} = {depreciation}"
                            )
                            break
        ...
        try:
            ...
            if net_income is None:
                logger.error(
                    f"None of the expected net income columns found for FCF calculation. Tried: {net_income_columns}. Available columns: {list(income_stmt.columns)}"
                )
        ...
        logger.error(
            "Could not calculate free cash flow using any method. Please check logs for missing fields and alternatives checked."
        )
        return None
...
def _calculate_terminal_value(
    fcf: float, growth_rate: float, discount_rate: float
) -> float:
    """
...
def _calculate_equity_value(
    enterprise_value: float, balance_sheet: pd.DataFrame
) -> float:
    """
    ...
    try:
        ...
        for col in debt_columns:
            if col in balance_sheet.columns:
                total_debt = (
                    balance_sheet[col].iloc[0] if not balance_sheet[col].empty else 0
                )
...
def _get_shares_outstanding(
    fundamentals: dict, balance_sheet: pd.DataFrame = None
) -> float | None:
    """
                ...
                for col in shares_columns:
                    if col in balance_sheet.columns and not balance_sheet[col].empty:
                        ...
                        if not pd.isna(shares_value) and shares_value > 0:
                            logger.info(
                                f"Found shares in balance sheet column '{col}': {shares_value:,.0f}"
                            )
                            return float(shares_value)

            logger.warning(
                "Shares outstanding not found in fundamentals or balance sheet"
            )
...
def _calculate_margin_of_safety(
    ...
    try:
        ...
        if (
            ...
            logger.warning(
                f"Invalid input(s) for margin of safety calculation: current_price={current_price}, intrinsic_value={intrinsic_value}"
            )
            return None
...
def get_projected_cash_flows(
    financial_data: pd.DataFrame, years: int = 5, conservative_factor: float = 0.8
) -> list[float]:
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully projected {len(projected_flows)} years of cash flows"
        )
        return projected_flows
...
def calculate_intrinsic_value_dcf(
    projected_flows: list[float], discount_rate: float, terminal_growth_rate: float
) -> float:
    """
...
def analyze_value_metrics(
    ...
    try:
        ...
        projected_flows = get_projected_cash_flows(
            financial_data, years=projection_years
        )
        ...
        intrinsic_value = calculate_intrinsic_value_dcf(
            projected_flows, discount_rate, terminal_growth_rate
        )

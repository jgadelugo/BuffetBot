...
from buffetbot.utils.validators import validate_financial_data
...
class GrowthMetrics:
    ...
    eps_avg_yoy: float | None
    fcf_avg_yoy: float | None


# Growth classification thresholds
GROWTH_THRESHOLDS = {"high": 0.15, "moderate": 0.05}  # 15% or higher  # 5% or higher


def _normalize_column_name(df: pd.DataFrame, possible_names: list[str]) -> str | None:
...
def calculate_cagr(start_value: float, end_value: float, periods: int) -> float:
    ...
    try:
        logger.debug(
            f"Calculating CAGR: start={start_value}, end={end_value}, periods={periods}"
        )

        if start_value <= 0 or end_value <= 0:
            logger.warning(
                f"Non-positive values in CAGR calculation: start={start_value}, end={end_value}"
            )
            return 0.0
    ...
    try:
        ...
        for metric_name, col_name in normalized_columns.items():
            if col_name and col_name in financials.columns:
                ...
                # Log metrics
                logger.info(
                    f"{metric_name.title()} - CAGR: {cagr:.2%}, Avg YOY: {avg_growth:.2%}"
                )
                ...
                if growth_std > 0.5:  # 50% standard deviation
                    logger.warning(
                        f"High volatility in {metric_name} growth (std: {growth_std:.2%})"
                    )
            else:
...
def analyze_growth_metrics(
    financials: dict[str, pd.DataFrame | dict[str, Any]],
...
def _calculate_revenue_growth(income_stmt: pd.DataFrame) -> float:
    ...
    """
    try:
        revenue_col = _normalize_column_name(
            income_stmt, ["Total Revenue", "Revenue", "revenue"]
        )
        if not revenue_col:
...
def _calculate_earnings_growth(income_stmt: pd.DataFrame) -> float:
    ...
    """
    try:
        earnings_col = _normalize_column_name(
            income_stmt, ["Net Income", "NetIncome", "net_income"]
        )
        if not earnings_col:
...
def _calculate_eps_growth(
    income_stmt: pd.DataFrame, fundamentals: dict[str, Any]
) -> float:
    ...
    try:
        ...
        eps_col = _normalize_column_name(
            income_stmt, ["EPS", "Earnings Per Share", "eps"]
        )
        if eps_col:
        ...
        net_income_col = _normalize_column_name(
            income_stmt, ["Net Income", "NetIncome", "net_income"]
        )
        shares_col = _normalize_column_name(
            income_stmt, ["Shares Outstanding", "shares_outstanding"]
        )
        ...
        if net_income_col and shares_col:
            ...
            if len(net_income) >= 2 and len(shares) >= 2:
                eps_current = (
                    net_income.iloc[0] / shares.iloc[0] if shares.iloc[0] != 0 else 0
                )
                eps_previous = (
                    net_income.iloc[1] / shares.iloc[1] if shares.iloc[1] != 0 else 0
                )

                if eps_previous != 0:
                    growth = (eps_current - eps_previous) / abs(eps_previous)
                    logger.info(
                        f"Calculated EPS growth from Net Income/Shares: {growth:.2%}"
                    )
                    return float(growth)
...
def _calculate_fcf_growth(cash_flow: pd.DataFrame) -> float:
    ...
    """
    try:
        fcf_col = _normalize_column_name(
            cash_flow, ["Free Cash Flow", "FCF", "free_cash_flow"]
        )
        if not fcf_col:
...
def _calculate_asset_growth(balance_sheet: pd.DataFrame) -> float:
    ...
    """
    try:
        assets_col = _normalize_column_name(

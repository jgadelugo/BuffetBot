...
# Use absolute imports to avoid conflicts with dashboard.utils
import sys
...
from buffetbot.utils.errors import DataError, ErrorSeverity
...
def standardize_column_names(df: pd.DataFrame, statement_type: str) -> pd.DataFrame:
    ...
    logger.info(
        f"Created empty standardized DataFrame with index: {type(standardized_df.index)}"
    )
    ...
    for col in df.columns:
        ...
        if matches:
            ...
            if std_name in standardized_df.columns:
                standardized_df[std_name] = standardized_df[std_name].fillna(df[col])
                logger.info(
                    f"  Fuzzy mapping: '{col}' -> '{std_name}' (via '{matches[0]}')"
                )
            else:
                standardized_df[std_name] = df[col]
                logger.info(
    ...
    unmapped = [
        ...
        and col not in name_mapping
        and not any(col in vars for vars in mappings.values())
    ]
    if unmapped:
    ...
    if fuzzy_matches:
        logger.warning(
            f"Fuzzy mapping suggestions for {statement_type} statement: {fuzzy_matches}"
        )
    ...
    for statement_type in ["income", "balance", "cash_flow"]:
        if statement_type in data:
            try:
                ...
                if df is None:
                    logger.warning(
                        f"None DataFrame received for {statement_type} statement"
                    )
                ...
                if df.empty:
                    logger.warning(
                        f"Empty DataFrame received for {statement_type} statement"
                    )
                    standardized_data[statement_type] = pd.DataFrame()
                    continue
                ...
                logger.error(
                    f"Error standardizing {statement_type} statement: {str(e)}"
                )

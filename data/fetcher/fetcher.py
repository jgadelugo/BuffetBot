...
class DataFetcher:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if hasattr(info, 'sharesOutstanding'):
                    fundamentals['shares_outstanding'] = getattr(info, 'sharesOutstanding', None)
                elif hasattr(info, 'impliedSharesOutstanding'):
                    fundamentals['shares_outstanding'] = getattr(info, 'impliedSharesOutstanding', None)
                
                fundamentals['latest_price'] = getattr(info, 'regularMarketPrice', None) or \
                                              getattr(info, 'currentPrice', None) or \
    ...
    def fetch_financial_data(self, ticker: str) -> Dict[str, Union[pd.DataFrame, Dict]]:
        """
        ...
        try:
            ...
            logger.info(f"Income statement type: {type(income_stmt)}, empty: {income_stmt is None or (hasattr(income_stmt, 'empty') and income_stmt.empty)}")
            ...
            logger.info(f"Balance sheet type: {type(balance_sheet)}, empty: {balance_sheet is None or (hasattr(balance_sheet, 'empty') and balance_sheet.empty)}")
            ...
            logger.info(f"Cash flow type: {type(cash_flow)}, empty: {cash_flow is None or (hasattr(cash_flow, 'empty') and cash_flow.empty)}")
            if cash_flow is not None and not cash_flow.empty:
                logger.info(f"Cash flow columns: {cash_flow.columns.tolist()}")
                # Ensure dates are in index
                if isinstance(cash_flow.columns[0], pd.Timestamp):
                    logger.info("Transposing cash flow statement to have dates as index")
            ...
            try:
                ...
                logger.info(f"Successfully fetched fundamentals: {list(fundamentals.keys())}")
            except Exception as e:
            ...
            for stmt_type, df in data_to_standardize.items():
                if df is not None and not df.empty:
                    ...
                    logger.info(f"{stmt_type} statement columns type: {[type(col) for col in df.columns]}")

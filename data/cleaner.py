...
from utils.validators import validate_financial_data, validate_price_data
from utils.errors import DataError, DataCleaningError, ErrorSeverity, handle_data_error

# Initialize logger
logger = setup_logger(__name__, "logs/data_cleaner.log")

def _parse_market_cap(value: Union[str, float, None]) -> Optional[float]:
    """
...
def clean_fundamentals(data: Dict[str, Union[float, str, None]]) -> Dict[str, Union[float, str, None]]:
    """
    ...
    try:
        ...
        numeric_metrics = [
            'pe_ratio', 'pb_ratio', 'eps', 'roe', 'dividend_yield', 'beta',
            'total_debt', 'total_equity', 'current_assets', 'current_liabilities',
            'interest_expense', 'ebit', 'gross_profit', 'operating_income',
            'net_income', 'revenue'
        ]
        ...
        missing_metrics = [metric for metric in required_metrics if metric not in cleaned]
        if missing_metrics:
        ...
        if all(k in cleaned and not pd.isna(cleaned[k]) for k in ['current_assets', 'current_liabilities']):
            cleaned['current_ratio'] = cleaned['current_assets'] / cleaned['current_liabilities']
            
        if all(k in cleaned and not pd.isna(cleaned[k]) for k in ['total_debt', 'total_equity']):
            cleaned['debt_to_equity'] = cleaned['total_debt'] / cleaned['total_equity']
            
        if all(k in cleaned and not pd.isna(cleaned[k]) for k in ['net_income', 'revenue']):
...
def clean_analyst_predictions(data: Dict[str, Union[float, str, None]]) -> pd.DataFrame:
    """
    ...
    try:
        ...
        numeric_cols = ['target_price', 'num_analysts', 'strong_buy', 'buy', 
        ...
        if missing.any():
            logger.warning(f"Missing values in predictions: {missing[missing > 0].to_dict()}")
...
def clean_financial_statement(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if isinstance(df_clean.index, pd.DatetimeIndex):
            ...
            df_clean.index = [str(idx).strip() if isinstance(idx, str) else idx for idx in df_clean.index]
            ...
            if df is not None and not df.empty:
                try:
                    ...
                    if not isinstance(df_clean.index, pd.DatetimeIndex):
                        try:
                            ...
                            try:
                                ...
                                logger.info(f"Successfully converted {statement_name} index to datetime using pattern matching")
                            except Exception as e2:
                                # If all conversion attempts fail, keep original index
                                logger.warning(f"Could not convert {statement_name} index to datetime: {str(e2)}")
                    ...
                    required_columns = {
                        ...
                        'balance_sheet': ['Total Assets', 'Total Current Assets', 'Total Liabilities', 'Total Current Liabilities', 'Total Stockholder Equity'],
                    ...
                    if statement_name in required_columns:
                        for col in required_columns[statement_name]:
                            if col not in df_clean.columns:
                                logger.warning(f"Missing required column {col} in {statement_name}")
                                df_clean[col] = 0
    ...
    try:
        ...
        if not income_stmt.empty and not balance_sheet.empty:
            ...
            if 'total_revenue' in latest_income and 'net_income' in latest_income:
                metrics['net_margin'] = latest_income['net_income'] / latest_income['total_revenue']
                
            if 'total_assets' in latest_balance and 'net_income' in latest_income:
                metrics['roa'] = latest_income['net_income'] / latest_balance['total_assets']
                
            if 'total_equity' in latest_balance and 'net_income' in latest_income:
                metrics['roe'] = latest_income['net_income'] / latest_balance['total_equity']
                
            if 'total_current_assets' in latest_balance and 'total_current_liabilities' in latest_balance:
                metrics['current_ratio'] = latest_balance['total_current_assets'] / latest_balance['total_current_liabilities']
                
            if 'total_debt' in latest_balance and 'total_equity' in latest_balance:
                metrics['debt_to_equity'] = latest_balance['total_debt'] / latest_balance['total_equity']

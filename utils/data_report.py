...
class DataCollectionReport:
    ...
    def _check_data_availability(self) -> dict[str, Any]:
        ...
        for statement_type in ["income_stmt", "balance_sheet", "cash_flow"]:
            ...
            logger.info(
                f"Available columns for {statement_type}: {list(statement_data.columns)}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Index type for {statement_type}: {type(statement_data.index)}"
                )
                logger.info(
                    f"First few index values for {statement_type}: {statement_data.index[:5].tolist()}"
                )

                # For balance sheet, we don't require a datetime index
                if statement_type == "balance_sheet":
                    logger.info(
                        "Balance sheet data found - using first row as latest data"
                    )
                    last_date = None
                else:
                    ...
                    if not isinstance(statement_data.index, pd.DatetimeIndex):
                        logger.info(
                            f"Index for {statement_type} is not datetime, skipping date processing"
                        )
                        last_date = None
                    else:
                        ...
                        if pd.isna(last_date):
                            logger.warning(
                                f"Could not determine last date for {statement_type} - all dates are invalid"
                            )
                            last_date = None
                        else:
                            logger.info(
                                f"Successfully determined last date for {statement_type}: {last_date}"
                            )
            except Exception as e:
                logger.error(f"Error processing date for {statement_type}: {str(e)}")
                logger.error(
                    f"Index values causing error: {statement_data.index[:5].tolist()}"
                )
                last_date = None
            ...
            try:
                for metric, value in fundamentals.items():
                    if isinstance(value, (int, float)):
                        ...
                        if value < 0:
                            logger.warning(
                                f"Negative value detected in fundamental metric {metric}: {value}"
                            )
            except Exception as e:
    ...
    def _analyze_impact(self, availability: dict[str, dict]) -> dict[str, list[str]]:
        ...
        try:
            ...
            if fundamentals:
                logger.info(
                    f"Available fundamental metrics: {list(fundamentals.keys())}"
                )
            ...
            if not availability.get("cash_flow", {}).get("available", False):
                impact["value_metrics"].extend(
                    ["Free Cash Flow", "Intrinsic Value (DCF)"]
                )
            elif "Free Cash Flow" in availability.get("cash_flow", {}).get(
                "missing_columns", []
            ):
                impact["value_metrics"].extend(
                    ["Free Cash Flow", "Intrinsic Value (DCF)"]
            ...
            if not availability.get("balance_sheet", {}).get("available", False):
                impact["health_metrics"].extend(
                    ["Current Ratio", "Debt-to-Equity Ratio", "Altman Z-Score"]
                )
            elif any(
                col in availability.get("balance_sheet", {}).get("missing_columns", [])
                for col in [
                    "Total Assets",
            ...
            if not availability.get("income_stmt", {}).get("available", False):
                impact["health_metrics"].extend(
                    ["Operating Margin", "Net Margin", "Piotroski Score"]
                )
            elif any(
                col in availability.get("income_stmt", {}).get("missing_columns", [])
            ...
            elif "Total Revenue" in availability.get("income_stmt", {}).get(
                "missing_columns", []
            ):
                impact["growth_metrics"].append("Revenue Growth")
            ...
            elif "Net Income" in availability.get("income_stmt", {}).get(
                "missing_columns", []
            ):
                impact["growth_metrics"].extend(["Earnings Growth", "EPS Growth"])
    ...
    def _generate_recommendations(
        self, availability: dict[str, dict], impact: dict[str, list[str]]
    ) -> list[str]:
        """
        ...
        for statement, status in availability.items():
            ...
            if missing_columns:
                ...
                value_impact = [
                    ...
                    if any(
                        col in metrics for metrics in impact.get("value_metrics", [])
                    )
                ]
                health_impact = [
                    col
                    for col in missing_columns
                    if any(
                        col in metrics for metrics in impact.get("health_metrics", [])
                    )
                ]
                growth_impact = [
                    col
                    for col in missing_columns
                    if any(
                        col in metrics for metrics in impact.get("growth_metrics", [])
                    )
                ]
        ...
        for statement, status in availability.items():
            if status.get("available", False) and "optional_columns" in status:
                missing_optional = [
                    col
                    for col in status.get("optional_columns", [])
            ...
            if status.get("available", False):
                ...
                if last_date:
                    try:
                        ...
                        logger.warning(
                            f"Error processing date for {statement}: {str(e)}"
                        )
    ...
    def _validate_data_structure(self) -> dict[str, dict]:
        ...
        validation = {
            "income_stmt": {"is_valid": False, "errors": [], "warnings": []},
            "balance_sheet": {"is_valid": False, "errors": [], "warnings": []},
            "cash_flow": {"is_valid": False, "errors": [], "warnings": []},
            "fundamentals": {"is_valid": False, "errors": [], "warnings": []},
        }
        ...
        if fundamentals:
            try:
                ...
                if not isinstance(fundamentals, dict):
                    ...
                    missing_metrics = [
                        m for m in required_metrics if m not in fundamentals
                    ]
                    if missing_metrics:
                    ...
                    for metric, value in fundamentals.items():
                        logger.info(
                            f"Checking metric {metric}: value={value}, type={type(value)}"
                        )
                    ...
                    validation["fundamentals"]["is_valid"] = (
                        len(validation["fundamentals"]["errors"]) == 0
                    )
            except Exception as e:
                logger.error(
                    f"Error validating fundamental metrics: {str(e)}", exc_info=True
                )
                validation["fundamentals"]["errors"].append(
        ...
        if income_stmt is not None:
            try:
                ...
                if not isinstance(income_stmt, pd.DataFrame):
                    ...
                    missing_cols = [
                        col for col in required_cols if col not in income_stmt.columns
                    ]
                    if missing_cols:
                        validation["income_stmt"]["errors"].append(
                    ...
                    validation["income_stmt"]["is_valid"] = (
                        len(validation["income_stmt"]["errors"]) == 0
                    )
            except Exception as e:
        ...
        if balance_sheet is not None:
            try:
                ...
                if not isinstance(balance_sheet, pd.DataFrame):
                    ...
                    missing_cols = [
                        col for col in required_cols if col not in balance_sheet.columns
                    ]
                    if missing_cols:
                        validation["balance_sheet"]["errors"].append(
                            f"Missing required columns: {', '.join(missing_cols)}"
                    ...
                    validation["balance_sheet"]["is_valid"] = (
                        len(validation["balance_sheet"]["errors"]) == 0
                    )
            except Exception as e:
        ...
        if cash_flow is not None:
            try:
                ...
                if not isinstance(cash_flow, pd.DataFrame):
                    ...
                    missing_cols = [
                        col for col in required_cols if col not in cash_flow.columns
                    ]
                    if missing_cols:
                        validation["cash_flow"]["errors"].append(
                            f"Missing required columns: {', '.join(missing_cols)}"
                    ...
                    validation["cash_flow"]["is_valid"] = (
                        len(validation["cash_flow"]["errors"]) == 0
                    )
            except Exception as e:
        ...
        try:
            ...
            report = {
                ...
                "summary": {
                    ...
                    "affected_metrics": sum(
                        len(metrics) for metrics in impact.values()
                    ),
                },
            }

            logger.info(
                f"Successfully generated data collection report with quality score: {quality_score:.1f}%"
            )
            return report

        except Exception as e:
            logger.error(
                f"Error generating data collection report: {str(e)}", exc_info=True
            )
            return {
                "timestamp": datetime.now().isoformat(),
                "error": str(e),
